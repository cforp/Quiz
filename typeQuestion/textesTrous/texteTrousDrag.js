	...
	for(var i=0;i<interaction.component[0].componentParam.length;i++){
		...
		$(interaction.component[0].componentParam[i].droppable).bind('dropout', function(event, ui){
			...
			if($(ui.draggable).attr('id') == $(this).find('> .draggable').attr('id')){
				...
				$(this).removeClass('droppable-plein');
...
texteTrouDrag.checkQuestionAnswered = function(interaction, userAnswer){	
	...
	for(var i=0;i<interaction.component[0].componentParam.length;i++){
		...
		if(interaction.component[0].componentParam[i].type == "trou"){	
			...
			if(!($(interaction.component[0].componentParam[i].droppable).hasClass('droppable-plein'))){
...
texteTrouDrag.getFeedback = function(answerArray, userAnswer){
	
	//for(var i=0; i < getObjectLength(userAnswer); i++){
	for(var i=0; i < getObjectLength(userAnswer); i++){
		for(var j=0; j < answerArray.length; j++){
			if(userAnswer[i] == answerArray[j].id){					
	...
	for(var i=0;i<interaction.component[0].componentParam.length;i++){
		if(interaction.component[0].componentParam[i].type == "trou"){			
			...
			if(interaction.component[0].componentParam[i].droppable.value == trouveDraggable.attr('id').substr(9)){
				trouveDraggable.addClass('ui-state-active');				
				
			}else{
				// mauvaise réponse
				trouveDraggable.addClass('ui-state-error');				
				...
				for(var j=0; j<answerArray.length;j++){
					if(answerArray[j].id == idReponse){
						// il faut vérifier avant si la classe texteTrouDragWA existe
						if($(interaction.component[0].componentParam[i].droppable).children('.texteTrouDragWA')!= undefined){
							$(interaction.component[0].componentParam[i].droppable).addClass('ui-state-active');
