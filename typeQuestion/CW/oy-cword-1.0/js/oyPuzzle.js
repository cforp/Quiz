...
oyCrosswordPuzzle.prototype.init = function(){ 
	...
	buf += "<td class='oyFooter' id='"+this.id+"oygFooterStatus' align='left'></td>";
	buf += "<td class='oyFooter' id='"+this.id+"oygFooterClock' align='right'></td>";  
	buf += "</tr>";
	...
	buf += "<tr><td class='oyCopyright' colspan='2' align='center'><a class='oysTextLink' id='"+this.id+"oygCopyright' href=''>" + cr + "</a></tr>";
	buf += "</table>"; 
	document.getElementById(this.id+"oygFooter").innerHTML = buf;
	
	document.getElementById(this.id+"oygCopyright").onclick = function(){
...
oyCrosswordPuzzle.prototype.render = function(){
	...
	for (var i=0; i < this.h; i++){	
		...
		for (var j=0; j < this.w; j++){	
			row += "<td class='oyCellEmpty' id='"+this.id+"oyCell" + j + "_" + i + "'></td>";
		} 
		buf += row + "</tr>";
	}	  
	buf += "</table>"; 	  
	...
	/* 
	var target = document.getElementById(this.id+"oygPuzzleFooter");
	target.style.width = divWidth;
	*/
	/*
	var target = document.getElementById(this.id+"oygState");
	target.style.width = divWidth; 
	*/
	var target = document.getElementById(this.id+"oygPuzzle");
	target.style.width = divWidth;
	...
	var target = document.getElementById(this.id+"oygListH");
	target.innerHTML = "";			
	target.className = "oyPanelDivHidden";
	
	var target = document.getElementById(this.id+"oygListV");
	target.innerHTML = "";
	target.className = "oyPanelDivHidden";		
}
...
oyCrosswordPuzzle.prototype.renderVert = function(clue){
	for (var i=0; i < clue.len; i++){	
		var key = this.id+"oyCell" + clue.xpos + "_" + (clue.ypos + i);
...
oyCrosswordPuzzle.prototype.renderHorz = function(clue){
	for (var i=0; i < clue.len; i++){	
		var key = this.id+"oyCell" + (clue.xpos + i) + "_" + clue.ypos
		var cell = document.getElementById(key);
		cell.className = "oyCellFull";
	}
}
...
oyCrosswordPuzzle.prototype.fillIn = function(cell, x, y, i, idx, dir){
	...
	cell.innerHTML = "<input id='"+this.id+"oyInput" + x + "_" + y + "' class='oyCellInput' autocomplete='on' type='text' size='1' maxlength='1' value=''>";	
}
...
oyCrosswordPuzzle.prototype.bind = function(){	
	...
	this.inputCache = new oyGridElementCache(this.w, this.h, this.id+"oyInput");
...
oyCrosswordPuzzle.prototype.focusCellList = function(all, focused){ 
	for (var i=0; i < all.length; i++){
		var cell = document.getElementById(this.id+"oyCell" + all[i].x + "_" + all[i].y);			
...
oyCrosswordPuzzle.prototype.focusNewCell = function(x, y, focus, clue){
	...
	var target = document.getElementById(this.id+"oyCell" + x + "_" + y);			
	if (target != null){   
...
oyCrosswordPuzzle.prototype.unfocusOldCell = function(){ 
	var target = document.getElementById(this.id+"oyCell" + this.xpos + "_" + this.ypos);			
	if (target != null){  
