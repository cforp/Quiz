...
oyClueList.prototype.render = function(){
	...
	for (var i=0; i < this.clues.length; i++){	
		...
		buf += "<tr><td class='oyListNormal' id='" + this.id + this.ns + i + "'><b>" + (i + 1) + ".</b> " + this.clues[i].clue + "</td></tr>";		
	}	  
	buf += "</table>";
...
oyClueList.prototype.bind = function(){
	for (var i=0; i < this.clues.length; i++){	
		var elem = document.getElementById(this.id + this.ns + i);
...
oyClueList.prototype.selectItem = function(idx){
	if (this.selIdx != - 1){
		document.getElementById(this.id + this.ns + this.selIdx).className = "oyListNormal";
	}	
	if (idx != -1){
		document.getElementById(this.id +this.ns + idx).className = "oyListSel";
	}
	this.selIdx = idx;
} 
